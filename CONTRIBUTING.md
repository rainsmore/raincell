...
When you view the notebook on GitHub or clone a local copy, you will see that, before developing each function, there are cells containing a few lines of code showing the expected output. Once the development produces the expected result, we combine the previous lines into a single function that can be imported from the library. Although this violates the 'Don't Repeat Yourself' (DRY) principle, this is a deliberate design choice. Just as solving a problem with pen and paper involves gradually and step by step developing the solution, we believe this approach is equally useful for developing algorithms in their computational form. This makes the reasoning easier to understand and review, both for oneself and for others. Additionally, you can download the notebook containing the relevant section and use it as a starting point.
...
After clonning the repo, make sure raincell package is installed in development mode
```sh
$ pip install -e .
```
...
Then make changes under nbs/ directory and commit them.

If for some reason you cannot (or you don't want) to install the pre-commit hooks, please run the following command **before each commit**:
```sh 
$ nbdev_prepare
```
